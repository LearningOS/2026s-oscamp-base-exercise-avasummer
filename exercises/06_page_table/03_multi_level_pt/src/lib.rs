...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in [2, 1] {
            ...
            let pte = self.nodes.get(&current_ppn).unwrap().entries[idx];

            let next_ppn = if pte & PTE_V == 0 {
                let new_ppn = self.alloc_node();
                self.nodes.get_mut(&current_ppn).unwrap().entries[idx] =
        ...
        self.nodes.get_mut(&current_ppn).unwrap().entries[idx] = (target_ppn << PPN_SHIFT) | flags;
    }
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let next_ppn = if pte & PTE_V == 0 {
            let new_ppn = self.alloc_node();
            self.nodes.get_mut(&current_ppn).unwrap().entries[idx] = (new_ppn << PPN_SHIFT) | PTE_V;
            new_ppn
        } else {
            pte >> PPN_SHIFT
        };
